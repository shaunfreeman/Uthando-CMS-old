...
if ($this->authorize()):
	...
	$menuBar = array(
		'new_page' => '/'.$this->registry->component.'/new',
	);
	...
	if ($num_pages > 0):
		...
		foreach ($result as $row):
			...
			$data[$c] = array(
				$row->page,
				$row->mdate,
				'<a href="/'.$this->registry->component.'/edit/id-'.$row->page_id.'"  style="text-decoration:none;" ><img src="/images/24x24/Edit3.png" class="Tips" title="Edit Page" rel="Click to edit the '.$row->page.' page contents" /></a>',
				
				'<a href="/'.$this->registry->component.'/delete/id-'.$row->page_id.'" ><img src="/images/24x24/DeleteRed.png" class="Tips" title="Delete Page" rel="Click to delete the '.$row->page.' page" /></a>'
			);
