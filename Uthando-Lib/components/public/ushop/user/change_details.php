...
if (UthandoUser::authorize()):
	...
	$sql = "
		...
		FROM ".$this->registry->user."ushop_user_info
	...
	if ($form->validate()):
		...
		try
		{
			$res = $this->registry->db->update($values,$this->registry->user.'ushop_user_info', array('WHERE' => 'user_info_id='.$user->user_info_id));
		}
		...
		Uthando::go('/ushop/view/checkout');
