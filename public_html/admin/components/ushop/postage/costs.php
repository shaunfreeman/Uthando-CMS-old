...
if ($this->authorize()) {
	...
	if ($rows = $this->getResult('post_cost_id, post_level, zone, cost',$ushop->db_name.'post_costs', array($ushop->db_name.'post_zones', $ushop->db_name.'post_levels'), array('ORDER BY' => 'zone, post_level, cost'))) {
		...
		foreach ($rows as $row) {
			...
			$data[$c][] = '<a href="/ushop/postage/action-edit_cost/id-'.$row->post_cost_id.'"  style="text-decoration:none;" ><img src="/templates/'.$this->registry->template.'/images/24x24/Edit3.png" class="Tips" title="Edit Post Cost" rel="Click to edit this post cost." /></a>';
			$data[$c][] = '<a href="/ushop/postage/action-delete_cost/id-'.$row->post_cost_id.'" ><img src="/templates/'.$this->registry->template.'/images/24x24/Delete.png" class="Tips" title="Delete Post Cost" rel="Click to delete this post cost" /></a>';
