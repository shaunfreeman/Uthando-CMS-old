...
class HTML_Template extends HTML_Page
{
	...
	private function loadScript($script)
	{
		...
		foreach ($script as $value):
			if ($this->settings['general']['cache'] || $this->settings['general']['dbug'] || is_file($_SERVER['DOCUMENT_ROOT'].$value)):
	...
	{
		$template = ($template) ? $template : TEMPLATES . $this->settings['general']['name'] . '/index.html';
	...
	private function errorCheck()
	{
		if ($this->registry->errors):
			...
			if (is_file(TEMPLATES . $this->settings['general']['name'] . '/html/errors.html')):
				
				$message = file_get_contents(TEMPLATES . $this->settings['general']['name'] . '/html/errors.html');
