...
class UShop {
	...
	public function productList($rows, $num_rows)
	{
		...
		for ($n = 1; $n <= $num_rows; $n++):
			...
			for ($d = $start; $d < ($display + $start); $d++):
				if ($rows[$d]):
					$message1 = $message;
					$params['NAME'] = htmlentities($rows[$d]->name);
					...
					$json_array[] = array(
						...
						"description" => $this->clean_code($rows[$d]->description)
					);
					
					if (!$rows[$d]->image_status) $message1 = $this->removeSection($message1, 'image');
					
					if ($this->GLOBAL['catelogue_mode']) $message1 = $this->removeSection($message1, 'add_cart');
	...
	public function clean_code($code)
	{
		...
		return preg_replace(array_keys($search), array_values($search), $code);
	}
	...
	public function productDetails($row)
	{
		...
		if (!$row->image_status) $html = $this->removeSection($html, 'image');
					
		if ($this->GLOBAL['catelogue_mode']) $html = $this->removeSection($html, 'add_cart');
	...
	public function removeSection($message, $type)
	{
		...
		for ($n = 1; $n <= $num_rows; $n++):
			...
			for ($d = $start; $d < ($display + $start); $d++):
				if ($rows[$d]):
					...
					if (!$rows[$d]['category_image_status']) $message1 = $this->removeSection($message1, 'image');
	...
	public function retrieveCart()
	{
		if (UthandoUser::authorize()):
			...
			$cart = $this->getCart();
			return new UShopShoppingCart($this->registry, $cart);
		else:
